...
class CommunityChat: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UITextFieldDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate {

  let cellid = "cell"
    var user: Tenant2s?
    var staff: Staff2s?
    
    var MessageArray = [GroupChatMessages]()
    var containerViewBottomAnchor: NSLayoutConstraint?
    var CommunityCollectionView: UICollectionView?
    ...
    super.viewDidLoad()
        CommunityCollectionView = UICollectionView(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: UICollectionViewFlowLayout())
        CommunityCollectionView?.register(Communitychatcell.self, forCellWithReuseIdentifier: cellid)
        ...
        CommunityCollectionView!.translatesAutoresizingMaskIntoConstraints = false
        ...
        CommunityCollectionView!.alwaysBounceVertical = true
        CommunityCollectionView!.contentInset = UIEdgeInsets(top: 8, left: 0, bottom: 58, right: 0)
        CommunityCollectionView!.scrollIndicatorInsets = UIEdgeInsets(top: 0, left: 0, bottom: 50, right: 0)
        
        
        //collectionview constraints
        CommunityCollectionView!.widthAnchor.constraint(equalTo: view.widthAnchor).isActive = true
        CommunityCollectionView!.topAnchor.constraint(equalTo: view.topAnchor).isActive = true
        CommunityCollectionView!.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
        CommunityCollectionView!.centerYAnchor.constraint(equalTo: view.centerYAnchor).isActive = true
                    ...
                    if let videourl = url?.absoluteString {
                        ...
                        if let thumbnailImage = self.thumbnailImageForVideo(videourl2: urlfromcamera){
                            ...
                            self.uploadtofirebasestorage(image: thumbnailImage, completion: { (imageurl2) in
                                ...
                                let properties = ["imageurl" : imageurl2, "videourl" : videourl, "fromid" : fromid, "timestamp" : timeStamp, "imagewidth" : thumbnailImage.size.width , "imageheight" : thumbnailImage.size.height] as [String : Any]
    ...
    func sendVideoAsMessage(values: [String : Any]) {
        ...
        Childref.updateChildValues(values) { (error, ref) in
            ...
            let userMessageRef = Database.database().reference().child("groupusermessages").child(fromid)
    ...
    func sendmessage(){
        ...
        let timeStamp = NSDate().timeIntervalSince1970 as NSNumber
        let values = ["text" : textfield!.text!, "fromid" : fromid, "timestamp" : timeStamp] as [String : Any]
    ...
    @objc func keybooardWillShow(notification: NSNotification) {
        ...
        CommunityCollectionView!.scrollToItem(at: indexpath, at: .top, animated: true)
        ...
        CommunityCollectionView!.contentInset = UIEdgeInsets(top: 8, left: 0, bottom: keyboardHeight + 8 + tabbarHeight!, right: 0)
    ...
    func observeMessages() {
        ...
        let ref = Database.database().reference().child("groupmessages")
        ...
        print(snapshot.value)
                ...
                let message = GroupChatMessages(dictionary: dictionary)
            ...
            print(self.MessageArray.count)
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.CommunityCollectionView!.scrollToItem(at: indexpath, at: .bottom, animated: true)
            }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellid, for: indexPath) as? Communitychatcell
    
        cell?.CommunityChat = self
...
class Communitychatcell: UICollectionViewCell {
    ...
    @objc func handleZoomTap(_tapGesture: UITapGestureRecognizer) {
        ...
        if let imageView = _tapGesture.view as? UIImageView {
            CommunityChat?.performZoomInForStartingImageView(startingImageView: imageView)
        }}
